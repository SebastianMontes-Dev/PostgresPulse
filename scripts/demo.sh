...
curl -sf -H "$AUTH" -X POST "$BASE_URL/api/v1/fuentes/$fuente_id/probar" | jq -c .

paso 3 "Ejecutando analisis (8 chequeos) ..."
analisis_json="$(curl -sf -H "$AUTH" -X POST "$BASE_URL/api/v1/fuentes/$fuente_id/analizar")"
...
salud_json="$(curl -sf -H "$AUTH" "$BASE_URL/api/v1/fuentes/$fuente_id/salud")"
...
curl -sf -H "$AUTH" "$BASE_URL/api/v1/analisis/$analisis_id/exportar?formato=html" -o "$destino"
